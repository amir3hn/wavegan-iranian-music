...
class WaveGANGenerator(nn.Module):
    def __init__(
        ...
        assert slice_len in [16384, 32768, 65536, 131072, 262144] 
        ...
        elif slice_len == 131072:
           deconv_layers += [
                Transpose1dLayer(
                 (self.dim_mul * model_size) // 16,  # Input channels
                 (self.dim_mul * model_size) // 32,  # Output channels
                 25,
                 stride,
                 upsample=upsample,
                 use_batch_norm=use_batch_norm,
                ...
                Transpose1dLayer((model_size) // 2, num_channels, 25, 2, upsample=upsample),
        
            ]

        elif slice_len == 262144:
            deconv_layers += [
               Transpose1dLayer(
                 (self.dim_mul * model_size) // 16,  # Input channels
                 (self.dim_mul * model_size) // 32,  # Output channels
    ...
    def forward(self, x):
        ...
        for i, deconv in enumerate(self.deconv_list[:-1]):
            x = F.relu(deconv(x))
            if self.verbose:
                 print(f"After deconv layer {i + 1}: {x.shape}") 
...
class WaveGANDiscriminator(nn.Module):
    def __init__(
        ...
        self.fc_input_size = 128 * model_size
        if slice_len == 32768:
            conv_layers.append(
                Conv1D(
                    16 * model_size,
            ...
            self.fc_input_size = 240 * model_size
        elif slice_len == 65536:
            conv_layers.append(
                Conv1D(
                    16 * model_size,
...
if __name__ == "__main__":
    ...
    for slice_len in [16384, 32768, 65536,131072]:
